...
本轮描绘的是 Chanel 在 2026-08-15 法国与美国公开本地标价样本中的可见价格架构，法国以 EUR、美国以 USD 分开处理。分析分组固定为：Classic 11.12 与 Small Classic 属于 Classic 组，Mini Classic、Shopping Bag 与 Bowling Bag 属于进入/其他核心对照。研究状态为 **{validation_status}**，数据可以复算；结论强度取决于样本覆盖、Dior 未解析价格和历史来源冲突。

## 1. 观察范围与覆盖

当前接受观察共 {current_accepted} 条，其中 {current_numeric} 条为数值价格：{brand_summary_cn}。Dior 美国有 {dior_us_numeric} 条数值价格（共 {dior_us_accepted} 条接受观察），另有 {dior_us_unresolved} 条价格未解析并保留为缺失值，不做插补。数值覆盖率描述接受观察中可解析价格行的比例；官网覆盖率还需要完整 SKU 清单与抓取范围记录。颜色变体与近重复保留在原始视图，去变体视图用于检验价格间隔对重复行的敏感性。

补充配对审计提供了 {len(pairing_candidates)} 条属性匹配的方向性候选配对和 {len(pairing_cells)} 个补充属性单元。由于补充快照与竞品基线日期不同，跨品牌价格比较保持 `not_computed`，统一观察日期后再计算。最新独立刷新含 {len(wave6_refresh)} 行、{len(wave6_cells)} 个精确属性单元和 {wave6_strict_word_cn} 个严格主文单元；尺寸检查出现 {matched_size_text}，主文资格仍需同时满足属性、状态和独立家族样本门槛。
...
在同一市场、同一快照内，法国 Classic 组最低观察价为 {fr['classic_min']} EUR，进入/其他核心对照最高观察价为 {fr['entry_core_max']} EUR，观察间隔为 {fr['observed_gap']} EUR；两组中位数距离为 {fr['median_distance']} EUR，相对比值为 {fr['relative_ratio']}。美国对应观察值为 {us['classic_min']}、{us['entry_core_max']} 与 {us['observed_gap']} USD，中位数距离为 {us['median_distance']} USD，相对比值为 {us['relative_ratio']}。
...
Chanel 法国的历史比较覆盖共同观察年份（{common_years}），固定产品线为 CH-C01/CH-C02 与 CH-C03/CH-C04；每年输出两组中位数、绝对距离、相对比值和产品线数量，缺失年份按缺失状态保留。历史名称与当前锚点按产品线规则对应，主要历史价格来自二手表格，因此结果适合有限的方向性判断；逐年官方调价路径还需要一手价格记录与共同日期。Hermès Geta 法国 2023 年保留 EUR 4,550 与 EUR 5,550 两条来源分支，结论沿分支呈现。
